...
static void render_reset(void)
{
	...
	uint16_t render_width = render.src.width;
	...
	uint16_t maxsize_current_input = SCALER_MAXWIDTH / render_width;
	...
	if ((render_width * simpleBlock->xscale > SCALER_MAXWIDTH) ||
	...
	render_width *= xscale;
	const auto render_height = make_aspect_table(render.src.height, yscale, yscale);
	...
	gfx_flags = GFX_SetSize(render_width,
	                        render_height,
...
static void init_render_settings(Section_prop& secprop)
{
	...
	string_prop->Set_help(
	        ...
	        "which may result in a non-integer scaling factor in the other direction.\n"
